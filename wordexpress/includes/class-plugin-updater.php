...
class ZeroY_Plugin_Updater {
    ...
    public function __construct() {
        $this->plugin_slug = 'zeroy';
        $this->version = ZEROY_VERSION;
        $this->author = 'Yansir';
        $this->update_path = 'https://www.zeroy.app/api/wp-updates/plugins/check';
        $this->plugin_name = 'zeroy';
        $this->plugin_file = ZEROY_BASENAME;
        ...
        // 当 WP 清除更新缓存时（如用户点击"再次检查"），同步清除 zeroY 缓存
    ...
    public function filter_update_transient($transient) {
        ...
        $cached = get_transient('zeroy_update_data');
        $backoff = get_transient('zeroy_update_backoff');
        ...
        if ($remote_version === false) {
            // 设置失败退避1小时
            set_transient('zeroy_update_backoff', 1, HOUR_IN_SECONDS);
            return $transient;
        }

        // 设置缓存
        $ttl = $this->get_success_cache_ttl();
        set_transient('zeroy_update_data', $remote_version, $ttl);
    ...
    private function get_remote_version() {
        ...
        $args = array(
            ...
            'redirection' => 2,
            'headers' => array(
                'User-Agent' => 'ZeroY/' . $this->version . ' (' . home_url() . ') WP/' . get_bloginfo('version'),
            ),
        );
    ...
    private function get_remote_info() {
        $cache_key = 'zeroy_plugin_info';
        $backoff_key = 'zeroy_info_backoff';
        ...
        $url = 'https://www.zeroy.app/api/wp-updates/plugins/info/' . rawurlencode($this->plugin_slug);
        $args = array(
            'timeout' => 3,
            'headers' => array(
                'User-Agent' => 'ZeroY/' . $this->version . ' (' . home_url() . ') WP/' . get_bloginfo('version'),
    ...
    private function get_changelog() {
        $cache_key = 'zeroy_plugin_changelog_html';
        $backoff_key = 'zeroy_changelog_backoff';
        $cached = get_transient($cache_key);
        if ($cached !== false) {
            return $cached;
        }
        ...
        $url = 'https://www.zeroy.app/api/wp-updates/plugins/changelog/' . rawurlencode($this->plugin_slug);
        $args = array(
            'timeout' => 3,
            'headers' => array(
                'User-Agent' => 'ZeroY/' . $this->version . ' (' . home_url() . ') WP/' . get_bloginfo('version'),
    ...
    private function is_dev() {
        return (defined('WP_DEBUG') && WP_DEBUG && defined('ZEROY_DEV_MODE') && ZEROY_DEV_MODE);
    }
    ...
    /**
     * 清除 zeroY 更新缓存，使下次检查时重新请求远端
     */
    public function clear_update_cache() {
        delete_transient('zeroy_update_data');
        delete_transient('zeroy_update_backoff');
    }
}

// 初始化插件更新器
new ZeroY_Plugin_Updater();
