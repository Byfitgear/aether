...
class ZeroY_Unified_Proxy {
    ...
    private function load_config() {
        $config_file = ZEROY_PATH . 'includes/config/proxy-config.php';
        ...
        $this->proxies = apply_filters('zeroy_proxy_config', $this->proxies);
    }
    ...
    private function register_default_middleware() {
        ...
        $this->add_global_middleware(new ZeroY_Proxy_Auth_Middleware());
        
        // Cache middleware
        $this->add_global_middleware(new ZeroY_Proxy_Cache_Middleware());
        
        // Log middleware
        if (defined('ZEROY_DEBUG') && constant('ZEROY_DEBUG') && !defined('ZEROY_DISABLE_PROXY_LOG')) {
            $this->add_global_middleware(new ZeroY_Proxy_Log_Middleware());
        }
    }
    ...
    public function request($proxy_name, $endpoint, $args = []) {
        if (!isset($this->proxies[$proxy_name])) {
            return new WP_Error('invalid_proxy', __('Invalid proxy service', 'zeroy'));
        }
        
        if (!isset($this->proxies[$proxy_name]['endpoints'][$endpoint])) {
            return new WP_Error('invalid_endpoint', __('Invalid endpoint', 'zeroy'));
        }
    ...
    private function get_environment_from_url() {
        ...
        // 最后检查 ZEROY_DEV_MODE 常量（向后兼容）
        if (defined('ZEROY_DEV_MODE') && constant('ZEROY_DEV_MODE')) {
            return 'development';
        }
    ...
    private function send_request($request) {
        $args = [
            ...
            'sslverify' => apply_filters('zeroy_proxy_verify_ssl', true)
        ];
