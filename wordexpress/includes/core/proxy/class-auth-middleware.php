 ...
 */
class ZeroY_Proxy_Auth_Middleware implements ZeroY_Proxy_Middleware
{
    ...
    public function process($request, $next)
    {
        ...
        if (!$token) {
            return new WP_Error('auth_failed', __('Authentication token not found', 'zeroy'));
        }
    ...
    private function parse_auth_config($auth_config, $global_config = [])
    {
        ...
        if (is_string($auth_config)) {
            ...
            return [
                'type' => $parts[0],
                'token_source' => $parts[1] ?? $global_config['default_token_source'] ?? 'settings:zeroy_settings[api_token]'
            ];
        }
        ...
        if (is_array($auth_config)) {
            ...
            if (!isset($auth_config['token_source'])) {
                $auth_config['token_source'] = $global_config['default_token_source'] ?? 'settings:zeroy_settings[api_token]';
            }
            return $auth_config;
        }
        ...
        return [
            'type' => 'bearer',
            'token_source' => $global_config['default_token_source'] ?? 'settings:zeroy_settings[api_token]'
        ];
    }
    ...
    private function get_token($source)
    {
        ...
        switch ($type) {
            ...
            case 'config':
                ...
                $config = get_option('zeroy_api_tokens', []);
                return $config[$key] ?? null;

            default:
                return apply_filters('zeroy_proxy_get_token', null, $type, $key);
        }
    }
}