<?php
/**
 * Admin Bar zeroy 编辑入口
 * 
 * 在 Admin Bar 中显示 zeroy 编辑器的快速访问入口
 * 支持检测和显示页面的多个 zeroy 组件（内容、页眉、页脚等）
 *
 * @package zeroy
 * @subpackage Admin
 */
...
class ZeroY_Admin_Bar_Edit
{
    ...
    {
        // 获取当前页面的所有 zeroy 组件
        $zeroy_components = self::get_zeroy_components();

        if (empty($zeroy_components)) {
            return;
        }

        // 计算组件数量
        $component_count = count($zeroy_components);

        if ($component_count === 1) {
            // 只有一个组件，直接显示编辑链接
            $component = reset($zeroy_components);
            $wp_admin_bar->add_node([
                'id' => 'zeroy-edit',
                'title' => '🎨 使用 zeroY 编辑',
                'href' => $component['edit_url'],
                'meta' => [
                    'class' => 'zeroy-edit-button',
            ...
            $wp_admin_bar->add_node([
                'id' => 'zeroy-edit',
                'title' => sprintf('🎨 使用 zeroY 编辑 (%d)', $component_count),
                'href' => '#',
                'meta' => [
                    'class' => 'zeroy-edit-button zeroy-has-submenu',
                    'title' => '使用 zeroY 编辑的组件'
                ]
            ]);

            // 添加子菜单项
            foreach ($zeroy_components as $component) {
                $wp_admin_bar->add_node([
                    'id' => 'zeroy-edit-' . $component['type'],
                    'parent' => 'zeroy-edit',
     ...
     * @return array
     */
    private static function get_zeroy_components()
    {
        $components = [];
        ...
        if (is_singular()) {
            global $post;
            if ($post && get_post_meta($post->ID, '_zeroy_edited', true) === '1') {
                $components['content'] = [
                    'type' => 'content',
                    'title' => '页面内容',
                    'icon' => '📄',
    ...
    private static function get_template_components()
    {
        ...
        // 获取模板实例
        $zeroy_templates = ZeroY_Templates::getInstance();

        // 检查页眉
        $header_template = $zeroy_templates->get_active_template('header');
        if ($header_template) {
            $components['header'] = [
                'type' => 'header',
                'title' => '页眉模板',
        ...
        // 检查页脚
        $footer_template = $zeroy_templates->get_active_template('footer');
        if ($footer_template) {
            $components['footer'] = [
                'type' => 'footer',
                'title' => '页脚模板',
        ...
        if ($current_type) {
            $template = $zeroy_templates->get_active_template($current_type);
            if ($template) {
                // 动态生成模板信息
    ...
    private static function get_template_display_info($template_type)
    {
        ...
        // 解析动态模板类型
        $template_types_instance = ZeroY_Dynamic_Template_Types::getInstance();
    ...
    public static function add_styles()
    {
        ...
        <style>
            /* zeroy 编辑按钮样式 */
            #wp-admin-bar-zeroy-edit>.ab-item {
            ...
            #wp-admin-bar-zeroy-edit:hover>.ab-item {
                background-color: #2563eb !important;
                color: white !important;
            }
            
            /* 单个项目时可点击 */
            #wp-admin-bar-zeroy-edit:not(.zeroy-has-submenu) > .ab-item {
                cursor: pointer !important;
            }
            
            /* 有子菜单时父菜单不可点击 */
            #wpadminbar .zeroy-has-submenu > .ab-item {
                cursor: default !important;
            }

            /* 子菜单样式 */
            #wpadminbar .zeroy-edit-button .ab-submenu {
                background-color: #1e40af !important;
            }

            #wpadminbar .zeroy-edit-button .ab-submenu .ab-item {
                color: white !important;
                padding-left: 20px !important;
                cursor: pointer !important;
            }

            #wpadminbar .zeroy-edit-button .ab-submenu .ab-item:hover {
                background-color: #2563eb !important;
            }

            /* 移除默认的 before 伪元素 */
            #wp-admin-bar-zeroy-edit .ab-item:before {
                content: none !important;
            }

            /* 确保图标正确显示 */
            #wpadminbar .zeroy-edit-button .ab-item {
                line-height: 32px;
            }
