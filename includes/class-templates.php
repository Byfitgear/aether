 ...
 */
class Aether_Templates extends Aether_Base {
	 ...
	 * @var string
	 */
	const POST_TYPE = 'aether_template';
	 ...
	 * @var string
	 */
	const CONTENT_META_KEY = '_aether_template_content';

	/**
	 * Dynamic template types manager
	 *
	 * @var Aether_Dynamic_Template_Types
	 */
	...
	protected function init() {
		...
		$this->template_types_manager = Aether_Dynamic_Template_Types::getInstance();
	...
	public function save_template_data( $data ) {
		...
		// Save template meta
		update_post_meta( $result, '_aether_template_type', sanitize_key( $data['type'] ) );
	...
	private function get_template_file( $type, $template ) {
		...
		$GLOBALS['aether_template_content'] = $template['content'];
		
		// Return our generic template renderer file
		return AETHER_PATH . 'templates/template-renderer.php';
	}
