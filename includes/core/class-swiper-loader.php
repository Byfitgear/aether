...
class Swiper_Loader {
     ...
     */
    private const META_KEY = '_aether_needs_swiper';
    ...
    public static function init(): void {
        ...
        add_action('aether_content_saved', [__CLASS__, 'on_content_saved']);
        add_action('aether_template_saved', [__CLASS__, 'on_template_saved']);
    }
    ...
    public static function on_content_saved(array $data): void {
        ...
        // Skip templates - they trigger aether_template_saved hook separately
        // Both Content API and Template API will trigger aether_template_saved for templates
        if ($post_type === 'aether_template') {
            if (defined('WP_DEBUG') && WP_DEBUG && defined('WP_DEBUG_LOG') && WP_DEBUG_LOG) {
                error_log(sprintf(
                    '[Swiper Loader] aether_content_saved 收到 template #%d，跳过（由 aether_template_saved 处理）',
                    $post_id
                ));
            }
            return;
    ...
    public static function enqueue_swiper(): void {
        ...
        wp_add_inline_script(
            'swiper',
            "window.dispatchEvent(new CustomEvent('aether:swiper:ready'));",
            'after'
        );
    }
