...
class Aether_Cache_Helper
{
     ...
     * @return void
     */
    public static function clear_aether_cache()
    {
        $aether_options = [
            'aether_settings',
            'aether_font_settings',
            'aether_font_css',
            'aether_font_formats',
            'aether_use_smart_css',
            'aether_smart_css_compiled_at',
            'aether_smart_css_stats',
        ];

        // 清理模板 CSS 缓存（动态生成的选项名）
        global $wpdb;
        $template_options = $wpdb->get_col(
            "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE 'aether_template_css_%'"
        );

        if ($template_options) {
            $aether_options = array_merge($aether_options, $template_options);
        }

        self::clear_option_cache($aether_options);
    }
        ...
        try {
            ...
            if (function_exists('rocket_clean_post')) {
                try {
                    ...
                    error_log('[aether] WP Rocket clear_post_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('LiteSpeed\\Purge')) {
                try {
                    ...
                    error_log('[aether] LiteSpeed Cache clear_post_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (function_exists('w3tc_flush_post')) {
                try {
                    ...
                    error_log('[aether] W3 Total Cache clear_post_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (function_exists('wp_cache_post_change')) {
                try {
                    ...
                    error_log('[aether] WP Super Cache clear_post_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('WpFastestCache') && method_exists('WpFastestCache', 'singleDeleteCache')) {
                try {
                    ...
                    error_log('[aether] WP Fastest Cache clear_post_cache failed: ' . $e->getMessage());
                }
            }
            ...
            error_log('[aether] clear_post_cache unexpected error: ' . $e->getMessage());
        }
    ...
    {
        // 防抖动：5秒内只清理一次
        $transient_key = 'aether_page_cache_cleared';
        ...
        try {
            ...
            if (function_exists('rocket_clean_domain')) {
                try {
                    ...
                    error_log('[aether] WP Rocket clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('LiteSpeed\\Purge')) {
                try {
                    ...
                    error_log('[aether] LiteSpeed Cache clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (function_exists('w3tc_flush_all')) {
                try {
                    ...
                    error_log('[aether] W3 Total Cache clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (function_exists('wp_cache_clear_cache')) {
                try {
                    ...
                    error_log('[aether] WP Super Cache clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (function_exists('wpfc_clear_all_cache')) {
                try {
                    ...
                    error_log('[aether] WP Fastest Cache clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('autoptimizeCache')) {
                try {
                    ...
                    error_log('[aether] Autoptimize clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('WP_Hummingbird')) {
                try {
                    ...
                    error_log('[aether] Hummingbird clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (function_exists('sg_cachepress_purge_cache')) {
                try {
                    ...
                    error_log('[aether] SG Optimizer clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('WpeCommon')) {
                try {
                    ...
                    error_log('[aether] WP Engine clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('Kinsta\\Cache') && isset($kinsta_cache)) {
                try {
                    ...
                    error_log('[aether] Kinsta clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('FlywheelNginxCompat')) {
                try {
                    ...
                    error_log('[aether] Flywheel clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('PagelyCachePurge')) {
                try {
                    ...
                    error_log('[aether] Pagely clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (class_exists('Pressable_Automated_Cache_Purge')) {
                try {
                    ...
                    error_log('[aether] Pressable clear_page_cache failed: ' . $e->getMessage());
                }
            }
            ...
            if (function_exists('nitropack_sdk_purge')) {
                try {
                    ...
                    error_log('[aether] NitroPack clear_page_cache failed: ' . $e->getMessage());
                }
            ...
            // 通用钩子
            do_action('aether_clear_page_cache');

            // 记录日志
            if ($success) {
                $plugins = self::get_active_cache_plugins();
                error_log(sprintf(
                    '[aether] Page cache cleared via: %s',
                    implode(', ', $plugins) ?: 'None (no cache plugin detected)'
                ));
            } else {
                error_log('[aether] Warning: No cache plugins detected, page cache not cleared');
            }
        } finally {
            // 只有成功执行后才设置防抖标记
            if ($success) {
    ...
    public static function clear_opcache()
    {
        ...
        if (function_exists('opcache_reset')) {
            ...
            if ($success) {
                error_log('[aether] OPcache cleared successfully');
            } else {
                error_log('[aether] Warning: Failed to clear OPcache');
            }
        }
