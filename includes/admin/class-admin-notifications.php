...
class Aether_Admin_Notifications {
    ...
    public function display_admin_notices() {
        ...
        // Note: aether settings page uses React component for notifications
        $allowed_screens = [
            'dashboard',
            'plugins',
            'update-core',
        ...
        if (!in_array($screen->id, $allowed_screens, true)) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[Aether Admin Notifications] Screen not allowed: ' . $screen->id);
            }
            return;
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Aether Admin Notifications] Got ' . count($notifications) . ' notifications');
            error_log('[Aether Admin Notifications] Notifications: ' . wp_json_encode($notifications));
        }
        ...
        // (Other notifications will be shown in aether settings/editor)
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Aether Admin Notifications] Rendered ' . $high_priority_count . ' high priority notifications');
        }
    }
    ...
    private function render_notice($notification) {
        ...
        ?>
        <div class="notice notice-<?php echo esc_attr($notice_type); ?> is-dismissible aether-remote-notice"
             data-notification-id="<?php echo $id; ?>">
            <p>
                <?php if ($title): ?>
                    <strong>[aether] <?php echo $title; ?></strong>
                    <br>
                <?php else: ?>
    ...
    public function enqueue_dismiss_script() {
        ...
        $script = "
        jQuery(document).on('click', '.aether-remote-notice .notice-dismiss', function() {
            var notice = jQuery(this).closest('.aether-remote-notice');
            var notificationId = notice.data('notification-id');

            if (!notificationId) return;

            jQuery.post(ajaxurl, {
                action: 'aether_dismiss_notification',
                notification_id: notificationId,
                nonce: '" . wp_create_nonce('aether-editor') . "'
            });
        });
        ";
...
// Initialize
Aether_Admin_Notifications::getInstance();
