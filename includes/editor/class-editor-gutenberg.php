...
class WordExpress_Editor_Gutenberg extends WordExpress_Base
{
    ...
    public function handle_gutenberg_editor()
    {
        ...
        if ($is_we_edited) {
            $editor_url = esc_url(add_query_arg(['page' => 'wordexpress-editor', 'post_id' => $post->ID], admin_url('admin.php')));
            $script = "wp.domReady(function(){
                ...
                if(editor){
                    ...
                    +'<div class=\"we-gutenberg-notice-content\"><h2>' + '<?php echo esc_js(__('此页面使用 WordExpress 编辑器创建', 'wordexpress')); ?>' + '</h2>'
                    +'<p>' + '<?php echo esc_js(__('要继续编辑此页面，请使用 WordExpress 编辑器。', 'wordexpress')); ?>' + '</p>'
                    +'<a href=\"' + '<?php echo esc_js($editor_url); ?>' + '\" class=\"button button-primary button-hero\">' + '<?php echo esc_js(__('打开 WordExpress 编辑器', 'wordexpress')); ?>' + '</a>'
                    +'<div class=\"warning\"><p>' + '<?php echo esc_js(__('警告：返回 WordPress 编辑器可能会破坏页面布局', 'wordexpress')); ?>' + '</p>'
                    +'<button type=\"button\" class=\"button button-link-delete\" onclick=\"if(confirm(\\'' . esc_js(__('确定继续？', 'wordexpress')) . '\\')){weRemoveEditFlag(' . $post->ID . ');}\">' + '<?php echo esc_js(__('返回 WordPress 编辑器', 'wordexpress')); ?>' + '</button></div></div>';
                    document.body.appendChild(notice);
                }
            });
            function weRemoveEditFlag(postId){wp.apiFetch({path:\"/wp/v2/posts/\"+postId,method:\"POST\",data:{meta:{_wordexpress_edited:\"\"}}}).then(function(){window.location.reload();});}";
