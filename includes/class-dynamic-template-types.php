...
/**
 * Aether_Dynamic_Template_Types class
 * 
 * @method WP_Post_Type[] get_post_types()
 * @method WP_Taxonomy[] get_taxonomies()
 */
class Aether_Dynamic_Template_Types extends Aether_Base
{
	...
	public function get_template_types()
	{
        ...
        $template_types['core'] = array(
            'label' => __('核心模板', 'aether'),
            'types' => array(
                '404' => __('404页面', 'aether'),
                'search' => __('搜索结果页', 'aether'),
                // Static site front page (a Page set as homepage)
                'front_page' => __('首页(静态页)', 'aether'),
                // Home when showing latest posts
                'home' => __('首页(最新文章)', 'aether'),
                'blog' => __('博客首页', 'aether'),
                'author' => __('作者页', 'aether'),
            )
        );

		// Layout parts
		$template_types['parts'] = array(
			'label' => __('布局部件', 'aether'),
			'types' => array(
				'header' => __('页眉', 'aether'),
				'footer' => __('页脚', 'aether'),
			)
		...
		$excluded_post_types = apply_filters('aether_excluded_post_types', array(
			'attachment',
			'aether_template', // Our own template post type
		));
		...
		$excluded_taxonomies = apply_filters('aether_excluded_taxonomies', array(
			'post_format',
		 ...
		 */
		return apply_filters('aether_template_types', $template_types);
	}
	...
	{
		require_once AETHER_PATH . 'includes/services/template/class-template-type-detector.php';
		return Aether_Template_Type_Detector::detect_current_page_type();
	}
	...
	{
		require_once AETHER_PATH . 'includes/services/template/class-template-preview.php';
        ...
        if (empty($context['subtype'])) {
            switch ($context['type']) {
                case '404':
                    $result['url'] = Aether_Template_Preview::generate_404_url();
				...
				case 'author':
					...
					if (!empty($authors)) {
						...
						$result['message'] = __('没有找到有文章的作者', 'aether');
					}
					break;
		...
		switch ($context['type']) {
			case 'single':
				$post = Aether_Template_Preview::get_preview_post($context['subtype']);
				if ($post) {
			...
			case 'taxonomy':
				$term = Aether_Template_Preview::get_preview_term($context['subtype']);
				if ($term) {
