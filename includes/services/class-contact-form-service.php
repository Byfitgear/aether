...
class Aether_Contact_Form_Service {
    ...
    public static function process_submission() {
        ...
        $n=sanitize_text_field($_POST['cf_name']??'');$e=sanitize_email($_POST['cf_email']??'');$s=sanitize_text_field($_POST['cf_subject']??'');$m=wp_kses_post($_POST['cf_message']??'');
        if(empty($n)||empty($e)||!is_email($e)||empty($m)){wp_safe_redirect(add_query_arg('cf_error',urlencode(__('Please fill all required fields.','aether')),wp_get_referer()));exit;}
        global $wpdb;$wpdb->insert($wpdb->prefix.self::TABLE_NAME,['name'=>$n,'email'=>$e,'subject'=>$s,'message'=>$m,'ip_address'=>$_SERVER['REMOTE_ADDR']??'','status'=>'new']);
        wp_mail(get_option('aether_cf_from_email',get_option('admin_email')),sprintf('[%s] %s',get_option('aether_cf_from_name',get_option('blogname')),$s?:__('New Contact Message','aether')),sprintf("Name: %s\nEmail: %s\nSubject: %s\n\nMessage:\n%s",$n,$e,$s?:'-',$m),['Content-Type: text/plain; charset=UTF-8']);
    ...
    public static function render_admin_page(){
        global $wpdb;$t=$wpdb->prefix.self::TABLE_NAME;
        if(isset($_POST['cf_update_status'])&&isset($_POST['cf_id'])){check_admin_referer('aether_cf_update_status');$wpdb->update($t,['status'=>sanitize_text_field($_POST['cf_status'])],['id'=>intval($_POST['cf_id'])]);set_transient('aether_cf_updated',true,30);}
        if(isset($_POST['cf_delete'])&&isset($_POST['cf_id'])){check_admin_referer('aether_cf_delete');$wpdb->delete($t,['id'=>intval($_POST['cf_id'])]);set_transient('aether_cf_deleted',true,30);}
        $pp=20;$pg=max(1,intval($_GET['paged']??1));$of=($pg-1)*$pp;$total=$wpdb->get_var("SELECT COUNT(*) FROM $t");$rows=$wpdb->get_results($wpdb->prepare("SELECT * FROM $t ORDER BY created_at DESC LIMIT %d OFFSET %d",$pp,$of));$pages=ceil($total/$pp);
        ...
        <?php if($pages>1)echo paginate_links(['total'=>$pages,'current'=>$pg]);endif;else:echo '<p>'.esc_html__('No submissions yet.','aether').'</p>';endif;?></div><?php
    }
}
